...
for case in diamond_MD20000_189x100x100 diamond_MD0_200x100x100 diamond_MD50000_174x101x100 diamond_MD100000_149x100x101 diamond_MD150000_131x100x104 diamond_MD200000_114x100x107
do
    ...
    for i in {01..10}
    do
        mpirun -np 20 python3 test_diamond_DFXM_mpi_workflow.py --casename ${case} -phi 0.00${i} -chi 0 -hkl 111 -sc 0.5
        wait
        mpirun -np 20 python3 test_diamond_DFXM_mpi_workflow.py --casename ${case} -phi 0.00${i} -chi 0 -hkl 004 -sc 0.25
        wait
        mpirun -np 20 python3 test_diamond_DFXM_mpi_workflow.py --casename ${case} -phi -0.00${i} -chi 0 -hkl 111 -sc 0.5
        wait
        mpirun -np 20 python3 test_diamond_DFXM_mpi_workflow.py --casename ${case} -phi -0.00${i} -chi 0 -hkl 004 -sc 0.25
