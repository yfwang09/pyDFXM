...
input_dict['nu'] = NU = 0.324       # Poisson's ratio (Al)
input_dict['b'] = bmag = 2.86e-10   # Burger's magnitude (m)
two_theta = 20.73                   # 2theta for Aluminum-(002) (deg)
...
savename = rvecname+'_'+bvecname+'_chi0.00060'
...
# For a finer resolution function
...
saved_res_fn = os.path.join('data', 'Res_qi_Al_001.npz')
...
if os.path.exists(savename + '.npz'):
    ...
    rt = rawdata['rt']
    print('loaded the data from %s'%savename)
    print('rt = %s'%rt)
else:
...
ax.legend()
fig.savefig(savename + '_contrast_Imin_Imax.png', transparent=True)
...
for i in range(len(rt)):
    ax.plot(phi_values*1000, Imax[i, :, 0].T - Imin[i, :, 0].T, 'o-', label=r'$\alpha = %d\degree$'%rt[i])
...
ax.set_xticks(phi_values[::2]*1000)
ax.set_ylabel('Contrast ($\Delta I = I_{\max} - I_{\min}$)')
ax.set_xlabel(r'$\phi(\times0.001\degree)$')
ax.legend()
fig.savefig(savename + '_contrast_phi.png', transparent=True)
...
ax.set_ylabel('Contrast ($\Delta I$)')
...
fig.savefig(savename + '_contrast_alpha.png', transparent=True)
plt.show()
